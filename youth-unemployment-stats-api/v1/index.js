...
/// GET /api/v1/youthUnemploymentStats ///
    app.get("/api/v1/youth-unemployment-stats",(req,res)=>{
        ...
        const youthUnemploymentStats_offset = parseInt(req.query.offset) || 0;
        const youthUnemploymentStats_limit = parseInt(req.query.limit) || 4;
    
        youthUnemploymentStats.find({}).skip(youthUnemploymentStats_offset).limit(youthUnemploymentStats_limit).toArray((err,youthUnemploymentStatsArray) =>{
            if(err)
                console.log("Error: " +err)
            res.send(youthUnemploymentStatsArray.map((c)=> {
                delete c._id;
                return c;
            }));   
        });
    ...
    app.get("/api/v1/youth-unemployment-stats/:country", (req,res) => {
        ...
        youthUnemploymentStats.find({"country": country}).skip(youthUnemploymentStats_offset).limit(youthUnemploymentStats_limit).toArray((err, youthUnemploymentStats_a)=>{
            if(err)
                console.log("Error: "+err);
            if(youthUnemploymentStats_a.length>0){
                res.send(youthUnemploymentStats_a.map((c) => {
                    delete c._id;
                    return c;
                }));
            }else{
    ...
    app.get("/api/v1/youth-unemployment-stats/:country/:year", (req,res) => {
        ...
        youthUnemploymentStats.find({"country": country, "year": year}).toArray((err, youthUnemploymentStats_a)=>{
            if(err)
                console.log("Error: "+err);
                if(youthUnemploymentStats_a.length>0){
                    delete youthUnemploymentStats_a[0]._id;
    ...
    //POST /api/v1/youth-unempoyment-stats/spain (ERROR METODO NO PERMITIDO)
    ...
    app.post("/api/v1/youth-unempoyment-stats/:country/:year", (req, res) => {
        res.sendStatus(405);
    });
    
    // PUT /api/v1/youth-unempoyment-stats/spain/2017
    app.put("/api/v1/youth-unempoyment-stats/:country/:year", (req,res) => {
        var country = req.params.country;
        var year = req.params.year;
        youthUnemploymentStats.find({"country": country, "year": year}).toArray((err, youthUnemploymentStats_a) => {
            if(err)
                ...
                console.log("Error: "+err);
            if(req.body.length!=5 || req.body.country==undefined || req.body.year==undefined || req.body.youth_unemployment==undefined
                || req.body.youth_unemployment_man==undefined || req.body.youth_unemployment_woman==undefined){
                res.sendStatus(400);
